...
export class MainView extends LitElement {
    ...
    static properties = {
        ...
        _azureEndpoint: { state: true },
        _azureDeployment: { state: true },
    ...
    async _loadFromStorage() {
        try {
            ...
            this._azureEndpoint = (await cheatingDaddy.storage.getAzureEndpoint().catch(() => '')) || '';
            this._azureDeployment = (await cheatingDaddy.storage.getAzureDeployment().catch(() => '')) || '';
    ...
    async _saveAzureEndpoint(val) {
        this._azureEndpoint = val;
        await cheatingDaddy.storage.setAzureEndpoint(val);
        this.requestUpdate();
    }

    async _saveAzureDeployment(val) {
        this._azureDeployment = val;
        await cheatingDaddy.storage.setAzureDeployment(val);
        this.requestUpdate();
    }
        ...
        return html`
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    .value=${this._azureEndpoint}
                    @input=${e => this._saveAzureEndpoint(e.target.value)}
                />
                <div class="form-hint">Azure OpenAI resource endpoint (optional)</div>
            </div>

            <div class="form-group">
                <label class="form-label">Azure API Key</label>
                <input type="password" placeholder="Optional" .value=${this._azureApiKey} @input=${e => this._saveAzureApiKey(e.target.value)} />
                <div class="form-hint">
                    <span class="link" @click=${() => this.onExternalLink('https://portal.azure.com')}>Azure Portal</span>
                </div>
            </div>

            <div class="form-group">
                <label class="form-label">Azure Deployment Name</label>
                <input type="text" placeholder="gpt-4o" .value=${this._azureDeployment} @input=${e => this._saveAzureDeployment(e.target.value)} />
                <div class="form-hint">Your model deployment name</div>
            </div>
